    ...
    <table class="table mt-4">
      ...
      <tbody>
        <tr v-for="(item) in coupons" :key="item.id">
          ...
          <td class="text-right" width="180">{{item.percent}}</td>
          <td class="text-right" width="180">{{item.code}}</td>
          <td class="text-right" width="180">{{item.due_date}}</td>
          <td>
            <span class="text-success" v-if="item.is_enabled" width="180">啟用</span>
    ...
    >
      <div class="modal-dialog" role="document">
        <div class="modal-content border-0">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">取消</button>
            <button type="button" class="btn btn-danger" @click="removeCoupon(tempCoupons.id)">確認刪除</button>
          </div>
        </div>
      </div>
    </div>
