      ...
      <tbody>
        <tr v-for="(item) in orders" :key="item.id">
          ...
          <td class="text-center">
            <button class="btn btn-outline-primary btn-sm" @click="openEditOrderModal(item)">編輯</button>

            <button class="btn btn-outline-primary btn-sm" @click="openDelOrderModal(item)">刪除</button>
          </td>
        </tr>
      </tbody>
    </table>
            ...
            <div class="row">
              <div class="col-sm-12">
                <div class="form-group">
                  ...
                  <input type="text" v-model="tempOrder.user.name" class="form-control" id="title" />
                  <label for="title">聯絡電話</label>
                  <input type="text" v-model="tempOrder.user.tel" class="form-control" id="title" />
                  <label for="title">電子信箱</label>
                  <input type="text" v-model="tempOrder.user.email" class="form-control" id="title" />
    ...
    >
      <div class="modal-dialog" role="document">
        <div class="modal-content border-0">
          ...
          <div class="modal-body">
            是否刪除
            <strong class="text-danger">序號 {{ tempOrder.num }}，購買人{{tempOrder.user.name}}的</strong> 訂單(刪除後將無法恢復)。
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">取消</button>
            <button type="button" class="btn btn-danger" @click="removeOrder(tempOrder.id)">確認刪除</button>
          </div>
        </div>
      </div>
    </div>
